...
function ReviewContent() {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', padding: '12px 16px', background: '#0A0A0A', border: '1px solid #2A2A2A', borderRadius: '10px', color: '#FEFEFE', fontSize: '14px', outline: 'none' };
      ...
      <div style={{ maxWidth: '560px', margin: '0 auto', padding: '100px 20px 60px' }}>
        ...
        {submitted ? (
          <div style={{ textAlign: 'center', paddingTop: '40px' }}>
            ...
            <p style={{ color: '#888', fontSize: '14px', marginBottom: '24px' }}>Your review has been submitted. We appreciate your feedback!</p>
          ...
          <>
            ...
            <p style={{ color: '#888', fontSize: '14px', marginBottom: '28px' }}>We'd love to hear about your experience at Eggs Ok</p>

            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {/* Rating */}
              <div>
                <label style={{ fontSize: '13px', color: '#888', display: 'block', marginBottom: '8px' }}>Rating *</label>
                <div style={{ display: 'flex', gap: '8px' }}>
                  {[1, 2, 3, 4, 5].map(star => (
                    <button key={star} type="button" onClick={() => setRating(star)} onMouseEnter={() => setHoverRating(star)} onMouseLeave={() => setHoverRating(0)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '32px', color: star <= (hoverRating || rating) ? '#FED800' : '#2A2A2A', transition: 'color 0.15s' }}>
                      ★
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label style={{ fontSize: '13px', color: '#888', display: 'block', marginBottom: '6px' }}>Order Type</label>
                <div style={{ display: 'flex', gap: '8px' }}>
                  {['Pickup', 'Delivery'].map(t => (
                    <button key={t} type="button" onClick={() => setOrderType(t)} style={{
                      flex: 1, padding: '10px', borderRadius: '8px', cursor: 'pointer',
                      background: orderType === t ? '#FED80020' : '#0A0A0A',
                      border: orderType === t ? '1px solid #FED800' : '1px solid #2A2A2A',
                      color: orderType === t ? '#FED800' : '#888', fontSize: '13px', fontWeight: '600',
                    }}>{t}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label style={{ fontSize: '13px', color: '#888', display: 'block', marginBottom: '6px' }}>Review Title *</label>
                <input required value={title} onChange={e => setTitle(e.target.value)} placeholder="Great food and fast service!" style={inputStyle} />
              </div>

              <div>
                <label style={{ fontSize: '13px', color: '#888', display: 'block', marginBottom: '6px' }}>Your Review *</label>
              ...
              <button type="submit" disabled={submitting} style={{
                padding: '14px', background: submitting ? '#1A1A1A' : '#FED800', border: 'none',
                borderRadius: '10px', color: submitting ? '#888' : '#000', fontSize: '15px', fontWeight: '700',
...
export default function ReviewPage() {
  return <Suspense fallback={<div style={{ background: '#000', minHeight: '100vh' }} />}><ReviewContent /></Suspense>;
}
