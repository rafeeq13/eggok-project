      ...
      <div style={{ maxWidth: '560px', margin: '0 auto', padding: '100px 20px 60px' }}>
        ...
        {submitted ? (
          <div style={{ textAlign: 'center', paddingTop: '40px' }}>
            ...
            <h1 style={{ fontSize: '28px', fontWeight: 700, fontFamily: "'Playfair Display', Georgia, serif", marginBottom: '8px' }}>Thank You!</h1>
            <p style={{ color: '#777777', fontSize: '16px', marginBottom: '24px' }}>Your review has been submitted. We appreciate your feedback!</p>
            <Link href="/order" style={{ padding: '8px 12px', background: '#E5B800', borderRadius: '10px', color: '#000', fontWeight: '700', fontSize: '16px', textDecoration: 'none' }}>Order Again</Link>
          </div>
        ) : (
          <>
            <h1 style={{ fontSize: '28px', fontWeight: 700, fontFamily: "'Playfair Display', Georgia, serif", marginBottom: '8px' }}>Leave a Review</h1>
            <p style={{ color: '#777777', fontSize: '16px', marginBottom: '28px' }}>We'd love to hear about your experience at Eggs Ok</p>

            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {/* Rating */}
              <div>
                <label style={{ fontSize: '12px', color: '#777777', display: 'block', marginBottom: '8px' }}>Rating *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ fontSize: '12px', color: '#777777', display: 'block', marginBottom: '6px' }}>Name *</label>
                  <input required value={name} onChange={e => setName(e.target.value)} placeholder="John Smith" style={inputStyle} />
                </div>
                <div>
                  <label style={{ fontSize: '12px', color: '#777777', display: 'block', marginBottom: '6px' }}>Email *</label>
                  <input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="john@gmail.com" style={inputStyle} />
                </div>
              </div>

              <div>
                <label style={{ fontSize: '12px', color: '#777777', display: 'block', marginBottom: '6px' }}>Order Type</label>
                <div style={{ display: 'flex', gap: '8px' }}>
                  {['Pickup', 'Delivery'].map(t => (
                    <button key={t} type="button" onClick={() => setOrderType(t)} style={{
                      ...
                      color: orderType === t ? '#1A1A1A' : '#777777', fontSize: '16px', fontWeight: '600',
                    }}>{t}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label style={{ fontSize: '12px', color: '#777777', display: 'block', marginBottom: '6px' }}>Review Title *</label>
                <input required value={title} onChange={e => setTitle(e.target.value)} placeholder="Great food and fast service!" style={inputStyle} />
              </div>

              <div>
                <label style={{ fontSize: '12px', color: '#777777', display: 'block', marginBottom: '6px' }}>Your Review *</label>
              ...
              <button type="submit" disabled={submitting} style={{
                padding: '8px 12px', background: submitting ? '#E5E5E5' : '#E5B800', border: 'none',
                borderRadius: '10px', color: submitting ? '#777777' : '#000', fontSize: '16px', fontWeight: '700',
